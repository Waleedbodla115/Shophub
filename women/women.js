/* ==============================================
   ...
   NOTE: addToCart(), getProductsByCategory() and
   updateCartCountBadges() come from the shared
   products.js — must be loaded BEFORE this file
   in women/index.html:
...
// Products are no longer hardcoded here — they come
// straight from the backend via products.js, so every
// product has a real MongoDB _id and Add to Cart /
// checkout always work correctly.
let womenProducts = [];
...
document.addEventListener('DOMContentLoaded', async () => {
    // Wait for products.js to finish loading real products
    // from the backend, then pull just the Women category —
    // and fetch this user's wishlist from the backend too.
    await Promise.all([fetchWishlist(), window.productsReady]);
    womenProducts = window.getProductsByCategory('Women');
